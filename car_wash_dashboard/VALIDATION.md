# 18.0.6.7.1 Arabic Translation Validation

- Regression scan requires zero visible English OWL literals except the Crystal Clean brand.
- All runtime `tr(...)` strings must exist in `i18n/ar_001.po`.
- Operational and Analytics UI, drill-down, empty states, warnings, tooltips, KPI notes, and controls are covered.
- RTL/LTR and Light/Dark behavior remain unchanged.
...
## 18.0.6.6 Interactive Analytics validation

1. Open **Analytics** and click each KPI card; confirm a right-side detail panel appears with only the relevant paid car-wash records.
2. Click each Daily Washes point, Popular Services bar, Station Utilization station, Top Customer, low-stock supply, Revenue Trend week, customer-mix segment, revenue-category legend item, and Recent Activity row.
3. Confirm the panel title/subtitle match the selected item and empty periods show a clean empty state rather than an error.
4. Use **Open in Odoo** on POS orders, MRP orders/work orders, customers, supplies, and work centers; confirm the correct form opens.
5. Change **Today / This Month** and confirm any open drill-down closes before loading the new period.
6. Confirm Analytics interactions do not create, write, unlink, start, finish, reserve, consume, invoice, or post any business record.

## 18.0.6.7 Identity / language / theme validation

1. Upgrade the existing addon in Staging; do not uninstall it.
2. Confirm the sidebar uses the supplied Crystal Clean logo and the text `Crystal Clean` / `WASH CENTER`.
3. With the Odoo user language set to English, confirm the dashboard remains English and LTR.
4. With the Odoo user language set to Arabic, confirm Operations, Analytics, station detail, KPI focus views, and analytics drill-down render in Arabic and RTL.
5. Confirm the theme button starts in Light mode on a fresh browser profile.
6. Toggle Dark mode and verify Operations and Analytics remain readable, including charts, cards, drill-downs, queue rows, and station details.
7. Reload the browser and confirm the chosen theme is restored when browser storage is available.
8. Toggle back to Light and confirm the existing approved light design is unchanged.
9. Confirm theme/language changes do not create, write, unlink, start, finish, reserve, consume, invoice, or post any Odoo business record.
