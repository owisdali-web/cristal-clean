...
## 18.0.6.0 — Automatic / Manual wash dashboards

- Added four Odoo menus: Overview, Automatic Wash, Manual Wash, Reports.
- Added operational `wash_method` (`automatic` / `manual`) to Sales Orders and Manufacturing Orders.
- Preserved the legacy `mrp.production.wash_type` field and all historical values; no old record is auto-classified.
- Dashboard RPC now accepts an optional wash-method filter so each mode has independent KPIs, queues, vehicles and wash-sales revenue.
- Added dedicated animated automatic-wash tunnel and manual-wash bay visuals.
- Replaced the old car image in the new sections with responsive vector car illustrations for small and large vehicles.
- Replaced the dashboard journey wording "inspection" with "drying" in the new UI.
- Reports page compares automatic and manual wash separately and opens filtered Odoo records.
