# 18.0.6.8.1 — Customer Lounge Display Polish

- Customer Lounge now carries the same `state.theme` value as the main dashboard and switches immediately between Light and Dark palettes.
- Theme class is attached directly to the lounge screen so the selected palette remains correct in browser Full Screen / TV Mode.
- Featured-car render is stabilized for large-screen viewing: centered aspect, larger safe image box, no floating animation, and a softer shadow to avoid the distorted/hovering appearance.
- Customer-facing typography is increased across the header, featured vehicle, journey stages, vehicle cards, footer, and empty states.
- Full Screen / TV Mode receives a second typography increase for long-distance readability.
- No MRP/POS/stock/accounting behavior is changed.
