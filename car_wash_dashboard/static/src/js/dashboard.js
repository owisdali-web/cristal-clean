...
class CarWashDashboard extends Component {
    ...
    async fetchData({ silent = false } = {}) {
        try {
            if (this.state.activeSection === "reports" || this.state.activeSection === "overview") {
    ...
    onSearchInput(ev) {
        this.state.searchTerm = String(ev?.target?.value || "").trim().toLowerCase();
    }

    get filteredActiveCars() {
        const cars = this.state.data.active_cars || [];
        const term = this.state.searchTerm;
        if (!term) return cars;
        return cars.filter((car) => [car.plate, car.customer, car.vehicle_model, car.service_name, car.current_stage]
            .filter(Boolean)
    ...
    get queueCars() {
        const cars = [...(this.state.data.active_cars || [])];
        const order = { waiting: 0, ready: 1, washing: 2, ready_delivery: 3, done: 4 };
        return cars.sort((a, b) => (order[a.status_code] ?? 9) - (order[b.status_code] ?? 9)).slice(0, 4);
    }

    get currentDateLabel() {
        try {
            return new Intl.DateTimeFormat("ar-LY", { weekday: "long", year: "numeric", month: "long", day: "numeric" }).format(new Date());
        } catch {
    ...
    get userInitials() {
        const name = String(this.state.data.user_name || "مستخدم").trim();
        return name.split(/\s+/).slice(0, 2).map((part) => part.charAt(0)).join("").toUpperCase();
    }

    get automaticRevenuePercent() {
        const automatic = Number(this.modeData("automatic").revenue_today || 0);
        const manual = Number(this.modeData("manual").revenue_today || 0);
        const total = automatic + manual;
        return total > 0 ? Math.round((automatic / total) * 100) : 0;
    }

    get manualRevenuePercent() {
        return (Number(this.modeData("automatic").revenue_today || 0) + Number(this.modeData("manual").revenue_today || 0)) > 0
            ? 100 - this.automaticRevenuePercent
            : 0;
    }

    revenueSplitStyle() {
        return `--cc-auto-share:${this.automaticRevenuePercent}%;`;
    }
    ...
    formatMoney(value, data = this.state.data) {
        const number = this.formatAmount(value);
