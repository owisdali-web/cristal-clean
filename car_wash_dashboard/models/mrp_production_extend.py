...
class MrpProduction(models.Model):
    ...
    def get_dashboard_data(self):
        ...
        if configuration_mode == 'fallback':
            warnings.append(_('Station types are using backward-compatible name detection. Configure dashboard station types.'))
        if len(real_station_payloads) != 10:
            warnings.append('The dashboard expects 10 real car-wash stations.')
        if automatic_count != 1:
            warnings.append(_('Configure exactly one Automatic station.'))
        if polishing_count != 1:
            warnings.append(_('Configure exactly one Polishing station.'))
        if general_count != 8:
            warnings.append(_('Configure eight General stations.'))
