...
class MrpProduction(models.Model):
    ...
    def _cw_vehicle_visual(self, raw_type, size):
        ...
        if raw in {'pickup', 'van', 'truck'}:
            return raw
        if raw in {'car', 'sedan'}:
            return 'car'
        return 'pickup' if size == 'large' else 'car'
    ...
    def _cw_vehicle_payload(self, mo, workorder=False, queue_item=False):
        ...
        raw_vehicle_type = self._cw_first_value(
            mo,
            ['x_cc_vehicle_size', 'vehicle_size', 'car_wash_vehicle_size'],
        ) or self._cw_first_value(
            sale,
            ['x_cc_vehicle_size', 'vehicle_size', 'car_wash_vehicle_size', 'vehicle_type'],
        )
        vehicle_size = normalize_vehicle_size(raw_vehicle_type, service_name)
        legacy_type = self._cw_first_value(sale, ['vehicle_type']) or raw_vehicle_type
    ...
    def get_dashboard_data(self):
        ...
        finished_today = self.search_count(
            base_domain + [
                ('state', '=', 'done'),
        ...
        return {
            'dashboard_version': '6.0-station-concept',
