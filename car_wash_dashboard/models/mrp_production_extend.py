...
class MrpProduction(models.Model):
    ...
    def _cw_dashboard_channel(self, company_id=None):
        """Company-scoped notification channel used only as a refresh signal.

        The bus payload contains no customer/vehicle data. The browser receives a small
        event then re-fetches the dashboard through normal Odoo access rights.
        """
    ...
    def _cw_mo_vehicle_payload(self, mo):
        ...
        workorders = workorders.sorted(
            key=lambda w: (
                getattr(w.operation_id, 'sequence', 0) if w.operation_id else 0,
                w.id,
            )
        )
        ...
        if mo.state == 'done':
            ...
            status_code, status_label = 'ready_delivery', 'جاهزة للتسليم'
        ...
        # Entry and delivery are synthetic visual nodes; the middle nodes are
        # the real mrp.workorder records from the real wash order.
        # ------------------------------------------------------------------
        raw_nodes = [{
            'key': 'entry',
            'workorder_id': False,
        ...
        for wo in workorders:
            if wo.state == 'done':
                node_state = 'done'
            elif current_wo and wo.id == current_wo.id:
                node_state = 'active' if wo.state == 'progress' else 'ready'
                current_node_key = 'wo_%s' % wo.id
            else:
                node_state = 'upcoming'
            ...
            raw_nodes.append({
                'key': 'wo_%s' % wo.id,
                'workorder_id': wo.id,
                'label': wo.name or 'مرحلة غسيل',
                'workcenter': wo.workcenter_id.name if wo.workcenter_id else '',
                'state': node_state,
                'icon': 'fa-tint' if 'غسيل' in (wo.name or '') else 'fa-cog',
            })

        delivery_state = 'done' if mo.state == 'done' else ('active' if mo.state == 'to_close' else 'upcoming')
        if mo.state in ('to_close', 'done'):
            current_node_key = 'delivery'
        raw_nodes.append({
            'key': 'delivery',
            'workorder_id': False,
            'label': 'جاهزة للتسليم',
        ...
        last_idx = max(len(raw_nodes) - 1, 1)
        journey_percent = 0.0
        seen_current = False
        for idx, node in enumerate(raw_nodes):
            node['position'] = round((idx / last_idx) * 100, 2)
            if node['key'] == current_node_key:
                journey_percent = node['position']
                seen_current = True
            if not next_stage and seen_current and idx + 1 < len(raw_nodes):
                next_stage = raw_nodes[idx + 1]['label']

        if mo.state == 'done':
            journey_percent = 100.0
            next_stage = ''
        elif mo.state == 'to_close':
            journey_percent = 100.0
            next_stage = 'التسليم'
        elif not current_wo and done_steps:
            # Fallback when the MO has completed steps but no active WO.
            journey_percent = round(min(95.0, ((done_steps + 1) / last_idx) * 100), 2)

        # Current-stage age and evidence-based delay. We only call a stage
        # delayed when Odoo itself has an expected duration and the active
        # execution has exceeded it. No arbitrary operational timeout is used.
        if current_wo:
            age_anchor = current_wo.date_start if current_wo.state == 'progress' and current_wo.date_start else current_wo.create_date
            if age_anchor:
                current_age_minutes = max(0, round((now - age_anchor).total_seconds() / 60))
            expected_minutes = round(current_wo.duration_expected or 0)
            stage_delayed = bool(
                current_wo.state == 'progress'
                and expected_minutes > 0
                and current_age_minutes > expected_minutes
            )

        station_id = current_wo.workcenter_id.id if current_wo and current_wo.workcenter_id else False
        station_name = current_wo.workcenter_id.name if current_wo and current_wo.workcenter_id else ''
        ...
        return {
            ...
            'workcenter_name': station_name,
            # Explicit aliases used by V4 live-motion components.
    ...
    def get_dashboard_data(self):
        ...
        # Prefer a vehicle physically washing now, then a vehicle ready for
        # delivery, then the next active vehicle. This feeds the large live
        # journey scene without changing any operational record.
        journey_car = (
            next((car for car in active_cars if car.get('status_code') == 'washing'), False)
            or next((car for car in active_cars if car.get('status_code') == 'ready_delivery'), False)
            or (active_cars[0] if active_cars else False)
        )
