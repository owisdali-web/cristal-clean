...
class MrpProduction(models.Model):
    ...
    wash_method = fields.Selection(
        ...
        string='نوع الغسيل',
        index=True,
        help='التصنيف التشغيلي للغسيل الآلي أو اليدوي. الحقل القديم wash_type محفوظ للتوافق فقط.',
    )
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get('wash_method') and vals.get('sale_line_id'):
                sale_line = SaleLine.browse(vals['sale_line_id']).exists()
                if sale_line and sale_line.order_id.wash_method:
                    vals['wash_method'] = sale_line.order_id.wash_method
        return super().create(vals_list)

    def write(self, vals):
        res = super().write(vals)
        if 'wash_method' in vals and not self.env.context.get('cw_sync_wash_method'):
            sale_orders = self.mapped('sale_line_id.order_id')
            if sale_orders:
                method = vals.get('wash_method') or False
                sale_orders.with_context(cw_sync_wash_method=True).write({'wash_method': method})
                siblings = self.search([
                    ('sale_line_id.order_id', 'in', sale_orders.ids),
                    ('id', 'not in', self.ids),
                ])
                if siblings:
                    siblings.with_context(cw_sync_wash_method=True).write({'wash_method': method})
        return res
    ...
    # ------------------------------------------------------------------
    @api.model
    def _cw_wash_domain(self, wash_method=False):
        domain = [('company_id', '=', self.env.company.id)]
        if 'x_cc_is_wash_order' in self._fields:
            domain.append(('x_cc_is_wash_order', '=', True))
        if wash_method in dict(WASH_METHODS):
            domain.append(('wash_method', '=', wash_method))
        return domain
    ...
    def _cw_mo_vehicle_payload(self, mo):
        ...
        return {
            ...
            'wash_method': mo.wash_method or False,
            'wash_method_label': dict(WASH_METHODS).get(mo.wash_method, 'غير محدد'),
    ...
    # ------------------------------------------------------------------
    @api.model
    def get_dashboard_data(self, wash_method=False):
        today_start, today_end, today = self._cw_day_bounds(0)
        now_str = fields.Datetime.to_string(fields.Datetime.now())
        company = self.env.company
        wash_method = wash_method if wash_method in dict(WASH_METHODS) else False
        base = self._cw_wash_domain(wash_method=wash_method)
        ...
        if wash_method:
            wo_base.append(('production_id.wash_method', '=', wash_method))
        ...
        if {'duration', 'duration_expected', 'date_finished'} <= set(Workorder._fields):
            ...
            if wash_method:
                finished_domain.append(('production_id.wash_method', '=', wash_method))
        ...
        if wash_method:
            wc_domain.append(('id', 'in', list(wc_map.keys()) or [0]))
        ...
        if wash_method:
            sale_domains['revenue_today'].append(('wash_method', '=', wash_method))
            sale_domains['pending_quotations'].append(('wash_method', '=', wash_method))
        ...
        if wash_method:
            wash_line_domain.append(('order_id.wash_method', '=', wash_method))
        ...
        if 'stock.scrap' in self.env:
            ...
            if 'production_id' in Scrap._fields:
                ...
                if wash_method:
                    scrap_domain.append(('production_id.wash_method', '=', wash_method))
        ...
        today_domain = [('create_date', '>=', today_start), ('create_date', '<', today_end)]
        method_counts = {
            'automatic': self.search_count(self._cw_wash_domain('automatic') + today_domain),
            'manual': self.search_count(self._cw_wash_domain('manual') + today_domain),
            'unclassified': self.search_count(self._cw_wash_domain() + today_domain + [('wash_method', '=', False)]),
        }

        # Vehicle-size summary used by the premium dashboard feature cards.
        # Keep the current four technical vehicle types intact and only group them
        # visually as small (car) vs large (truck/van/pickup).
        vehicle_counts = {'small': 0, 'large': 0}
        today_mos = self.search(kpi_domains['total_today'])
        for mo in today_mos:
            sale = mo.sale_line_id.order_id if mo.sale_line_id else self.env['sale.order']
            vehicle_type = sale.vehicle_type if sale and 'vehicle_type' in sale._fields else 'car'
            if vehicle_type in ('truck', 'van', 'pickup'):
                vehicle_counts['large'] += 1
            else:
                vehicle_counts['small'] += 1

        return {
            'dashboard_version': '7.0-premium-concept',
            'wash_method': wash_method or False,
            'wash_method_label': dict(WASH_METHODS).get(wash_method, 'كل أنواع الغسيل'),
