...
class MrpProductionBusinessAnalytics(models.Model):
    ...
    def _cw_analytics_order_row(self, order):
        ...
        return {
            'id': f'pos-{order.id}',
            'primary': order.name,
            'secondary': partner.display_name if partner else 'Walk-in customer',
            'detail': services or 'Car wash service',
    ...
    def get_business_analytics_detail(self, detail_type, key=False, period='today'):
        ...
        if detail_type in ('today_revenue', 'washes_today', 'average_ticket'):
            ...
            titles = {
                ...
                'washes_today': 'Total Washes Today',
                'average_ticket': 'Average Ticket',
            }
            summary = [
                {'label': 'Wash visits', 'value': str(len(rows))},
                {'label': 'Revenue', 'value': self._cw_analytics_money_text(revenue)},
            ]
            if detail_type == 'average_ticket':
                average = revenue / len(rows) if rows else 0.0
                summary.append({'label': 'Average', 'value': self._cw_analytics_money_text(average)})
            return self._cw_analytics_detail_payload(
                detail_type, titles[detail_type], 'Paid car-wash POS visits today.', rows, summary,
            )
        ...
        if detail_type == 'active_customers':
            ...
            for partner in customers:
                ...
                rows.append({
                    ...
                    'detail': f'{len(partner_orders)} wash visit(s) today',
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                'Active Customers',
                'Customers with paid car-wash visits today.',
                rows,
        ...
        if detail_type == 'daily_washes':
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                f'Daily Washes · {start_hour:02d}:00–{(start_hour + 3) % 24:02d}:00',
                'Paid car-wash visits in this time bucket.',
                rows,
                [{'label': 'Washes', 'value': str(len(rows))}],
            )
        ...
        if detail_type == 'popular_service':
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                product.display_name if product else 'Service',
                'Service usage in the selected analytics period.',
                rows,
                [
                    {'label': 'Quantity', 'value': f'{quantity:g}'},
                    {'label': 'Revenue', 'value': self._cw_analytics_money_text(revenue)},
                ],
            )
        ...
        if detail_type == 'station_overall':
            ...
            for station_row in station_data['stations']:
                rows.append({
                    ...
                    'secondary': station_row['station_type'].title(),
                    'detail': f"{round(station_row['busy_minutes'])} busy min / {round(station_row['capacity_minutes'])} capacity min",
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                'Station Utilization',
                'Utilization for all configured car-wash stations.',
                rows,
                [{'label': 'Overall utilization', 'value': f"{station_data['overall']}%"}],
            )
        ...
        if detail_type == 'station':
            ...
            if not station:
                return self._cw_analytics_detail_payload(detail_type, 'Station', 'Station not found.')
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                station.name,
                f"{station.car_wash_station_type.title()} station activity.",
                rows,
                [
                    {'label': 'Utilization', 'value': f"{util_row.get('utilization', 0)}%"},
                    {'label': 'Busy time', 'value': f"{round(util_row.get('busy_minutes', 0))} min"},
                    {'label': 'Wash operations', 'value': str(len(rows))},
                ],
            )
        ...
        if detail_type == 'top_customer':
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                partner.display_name if partner else 'Customer',
                'Car-wash history in the selected analytics period.',
                rows,
                [
                    {'label': 'Washes', 'value': str(len(rows))},
                    {'label': 'Revenue', 'value': self._cw_analytics_money_text(revenue)},
        ...
        if detail_type == 'supply':
            ...
            product = self.env['product.product'].sudo().browse(product_id).exists()
            if not product:
                return self._cw_analytics_detail_payload(detail_type, 'Supply', 'Supply not found.')
            ...
            rows = [{
                ...
                'detail': ', '.join(boms.mapped('display_name')[:5]) or 'No linked car-wash BoM',
                'metric': f'{available:g} available',
            ...
            return self._cw_analytics_detail_payload(
                ...
                'Current supply level and linked car-wash recipes.',
                rows,
                [
                    {'label': 'Available', 'value': f'{available:g} {product.uom_id.display_name if product.uom_id else ""}'.strip()},
                    {'label': 'Reorder minimum', 'value': f'{minimum:g}'},
                ],
            )
        ...
        if detail_type == 'revenue_week':
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                f'Revenue Trend · Week {week_index + 1}',
                month_bounds['label'],
                rows,
                [
                    {'label': 'Wash visits', 'value': str(len(rows))},
        ...
        if detail_type == 'customer_mix_overall':
            ...
            for partner in partners:
                ...
                segment = 'Returning' if partner.id in returning_ids else 'New'
                rows.append({
                    'id': f'partner-{partner.id}',
                    'primary': partner.display_name,
                    'secondary': segment,
                    'detail': f'{len(orders)} wash visit(s)',
            ...
            return self._cw_analytics_detail_payload(
                ...
                'Customer classification for the selected analytics period.',
                rows,
                [
                    {'label': 'Returning', 'value': str(len(partner_ids & returning_ids))},
                    {'label': 'New', 'value': str(len(partner_ids - returning_ids))},
                ],
            )
        ...
        if detail_type == 'customer_mix':
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                'Returning Customers' if segment == 'returning' else 'New Customers',
                'Customer mix for the selected analytics period.',
                rows,
                [{'label': 'Customers', 'value': str(len(rows))}],
            )
        ...
        if detail_type == 'revenue_category':
            ...
            return self._cw_analytics_detail_payload(
                detail_type,
                category.display_name if category else 'Other',
                'Monthly revenue for this service category.',
                rows,
                [
                    {'label': 'Orders', 'value': str(len(rows))},
                    {'label': 'Revenue', 'value': self._cw_analytics_money_text(revenue)},
                ],
            )
        ...
        if detail_type == 'recent_activity':
            ...
            if model_name == 'pos.order' and res_id:
                ...
                return self._cw_analytics_detail_payload(
                    detail_type, 'Payment received', order.name if order else '', rows,
                    [{'label': 'Record', 'value': order.name if order else 'Not found'}],
                )
            if model_name == 'mrp.production' and res_id:
                ...
                if mo:
                    ...
                    return self._cw_analytics_detail_payload(
                        detail_type, 'Wash Order Activity', mo.name, rows,
                        [{'label': 'State', 'value': mo.state}],
                    )
            return self._cw_analytics_detail_payload(detail_type, 'Recent Activity', 'Record not found.')

        return self._cw_analytics_detail_payload(
            detail_type,
            'Analytics Detail',
            'No drill-down is available for this item.',
        )
    ...
    def get_business_analytics_data(self, period='today'):
        ...
        popular_services = [
            {'product_id': product_id, 'name': service_names.get(product_id, 'Service'), 'value': round(value, 2)}
        ...
        revenue_trend = [
            {'label': f'Week {index + 1}', 'value': round(value, 2), 'week_index': index}
