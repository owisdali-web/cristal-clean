...
class MrpProductionReportCenter(models.Model):
    ...
    def _cw_report_operation_row(self, mo):
        ...
        report_stage = mo.state
        if mo.state in ('confirmed', 'progress'):
            report_stage = 'in_progress' if workorders.filtered(lambda item: item.state == 'progress') else 'waiting'
        return {
            'id': mo.id,
    ...
    @api.model
    def _cw_report_trend_bucket_key(self, date_order, bounds):
        local_dt = self._cw_analytics_local_datetime(date_order)
        if not local_dt:
            return False
        day_count = max(1, (bounds['end_local'].date() - bounds['start_local'].date()).days)
        if day_count == 1:
            return min(7, local_dt.hour // 3)
        index = (local_dt.date() - bounds['start_local'].date()).days
        return index if 0 <= index < day_count else False

    @api.model
    def _cw_report_transaction_rows(self, lines, bounds):
        rows = []
        for line in lines:
            order = line.order_id
            ...
            for payment in payments:
                name = payment.payment_method_id.name if payment.payment_method_id else _('Other')
                if name and name not in payment_methods:
                    payment_methods.append(name)
            if not payment_methods:
                payment_methods = [_('Unspecified')]
            ...
            rows.append({
                ...
                'customer': partner.display_name if partner else _('Walk-in'),
                'product_id': line.product_id.id,
                'service': line.product_id.display_name,
                'vehicle_size': normalize_vehicle_size('', line.product_id.display_name),
                'qty': round(float(getattr(line, 'qty', 0.0) or 0.0), 2),
                'unit_price': round(float(getattr(line, 'price_unit', 0.0) or 0.0), 2),
                'amount': round(self._cw_analytics_line_amount(line), 2),
                'payment_methods': payment_methods,
                'payment_method_label': ', '.join(payment_methods),
                'trend_bucket_key': self._cw_report_trend_bucket_key(order.date_order, bounds),
                'model': 'pos.order',
                'res_id': order.id,
            })
        return rows
    ...
    def _cw_report_revenue_trend(self, lines, bounds):
        ...
        for order in orders:
            key = self._cw_report_trend_bucket_key(order.date_order, bounds)
            if key is not False and 0 <= key < len(values):
                values[key] += amount_by_order[order.id]
        return [
            {'label': label, 'value': round(values[i], 2), 'bucket_key': i}
    ...
    def get_report_center_data(self, filters=None):
        ...
        operations = [self._cw_report_operation_row(mo) for mo in mos]
        supplies = [
            {**row, 'model': 'product.product', 'res_id': row.get('product_id')}
            for row in self._cw_analytics_low_supplies()
        ]
        ...
        for mo in mos:
            ...
            if mo.state == 'cancel':
                exceptions.append({'type': 'cancelled', 'severity': 'danger', 'reference': mo.name, 'detail': payload['plate'], 'message': _('Cancelled wash order'), 'model': 'mrp.production', 'res_id': mo.id})
                ...
                if elapsed > float(progress_wo.duration_expected) * 1.25:
                    exceptions.append({'type': 'overdue', 'severity': 'warning', 'reference': mo.name, 'detail': payload['plate'], 'message': _('Wash exceeds expected duration'), 'model': 'mrp.production', 'res_id': mo.id})
            elif mo.state in ('confirmed', 'progress') and payload.get('waiting_minutes', 0) and payload['waiting_minutes'] > 30:
                exceptions.append({'type': 'waiting', 'severity': 'warning', 'reference': mo.name, 'detail': payload['plate'], 'message': _('Waiting longer than 30 minutes'), 'model': 'mrp.production', 'res_id': mo.id})
        ...
        return {
            ...
            'transactions': self._cw_report_transaction_rows(lines, bounds),
