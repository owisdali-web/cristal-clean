...
class CarWashReportExportController(http.Controller):
    def _tr(self, text, **kwargs):
        return request.env._(text, **kwargs)

    def _is_arabic(self):
        lang = request.env.context.get('lang') or request.env.user.lang or ''
        return str(lang).lower().startswith('ar')

    def _localized_technical_label(self, value):
        raw = str(value or '').replace('_', ' ').strip()
        return self._tr(raw.title()) if raw else ''
    ...
    def _filter_summary(self, data):
        ...
        values = [self._tr('Period: %(period)s', period=data.get('period_label') or '-')]
        if filters.get('station_id'):
            values.append(self._tr('Station: %(station)s', station=option_name('stations', filters['station_id'])))
        if filters.get('service_id'):
            values.append(self._tr('Service: %(service)s', service=option_name('services', filters['service_id'])))
        if filters.get('customer_id'):
            values.append(self._tr('Customer: %(customer)s', customer=option_name('customers', filters['customer_id'])))
        if filters.get('vehicle_size'):
            values.append(self._tr('Vehicle Size: %(size)s', size=self._localized_technical_label(filters['vehicle_size'])))
        if filters.get('status'):
            values.append(self._tr('Status: %(status)s', status=self._localized_technical_label(filters['status'])))
        return ' | '.join(values)
    ...
    def export_xlsx(self, **params):
        ...
        workbook.set_properties({
            'title': tr('Crystal Clean - Management Report'),
            'subject': tr('Car wash management report - %(period)s', period=data.get('period_label') or '-'),
        ...
        title_fmt = workbook.add_format({'bold': True, 'font_size': 22, 'font_color': '#FFFFFF', 'bg_color': navy, 'align': text_align, 'valign': 'vcenter'})
        subtitle_fmt = workbook.add_format({'font_size': 10, 'font_color': '#DCEBFA', 'bg_color': navy, 'align': text_align, 'valign': 'vcenter'})
        ...
        section_fmt = workbook.add_format({'bold': True, 'font_size': 13, 'font_color': '#FFFFFF', 'bg_color': blue, 'align': text_align, 'valign': 'vcenter'})
        ...
        def setup_sheet(sheet, landscape=True):
            ...
            sheet.set_header('&LCrystal Clean&R' + tr('Management Report'))
            sheet.set_footer(f'&L{data.get("period_label", "")}&R{tr("Page")} &P / &N')
        ...
        def write_report_header(sheet, title, last_col):
            ...
            if logo:
                try:
                    ...
                    pass
            sheet.write(2, 0, tr('Generated By'), meta_label_fmt)
            sheet.write(2, 1, meta['generated_by'], meta_value_fmt)
            sheet.write(2, 2, tr('Generated At'), meta_label_fmt)
            sheet.write(2, 3, meta['generated_at'], meta_value_fmt)
            if last_col >= 5:
                sheet.write(2, 4, tr('Applied Filters'), meta_label_fmt)
                if last_col > 5:
            ...
            for rindex, item in enumerate(rows, start=start_row + 1):
                for col, key in enumerate(keys):
                    ...
                    if value is False or value is None:
                        value = ''
                    if key in ('station_type', 'type'):
                        value = self._localized_technical_label(value)
        ...
        # Executive Summary
        summary_name = tr('Executive Summary')[:31]
        revenue_name = tr('Revenue & Payments')[:31]
        summary_ref = summary_name.replace("'", "''")
        revenue_ref = revenue_name.replace("'", "''")
        ...
        summary.merge_range('B1:I2', tr('Crystal Clean - Management Report'), title_fmt)
        summary.merge_range('B3:I3', f"{tr('Executive Summary')} | {data.get('period_label', '')}", subtitle_fmt)
        logo = self._logo_path()
        if logo:
            try:
                ...
                pass
        summary.write('B5', tr('Generated By'), meta_label_fmt)
        summary.write('C5', meta['generated_by'], meta_value_fmt)
        summary.write('D5', tr('Generated At'), meta_label_fmt)
        summary.write('E5', meta['generated_at'], meta_value_fmt)
        summary.write('F5', tr('Applied Filters'), meta_label_fmt)
        summary.merge_range('G5:I5', meta['filters'], meta_value_fmt)
        kpis = data.get('kpis') or {}
        kpi_specs = [
            (tr('Total Cars'), 'total_cars', False), (tr('Finished Cars'), 'finished_cars', False), (tr('Waiting Cars'), 'waiting_cars', False),
            (tr('Total Revenue'), 'total_revenue', True), (tr('Average Ticket'), 'average_ticket', True), (tr('Unique Customers'), 'unique_customers', False),
        ]
        ...
        summary.merge_range('B15:I15', tr('Management Highlights'), section_fmt)
        highlight_rows = [
            (tr('Top Service by Revenue'), highlights['top_service']), (tr('Highest Utilization Station'), highlights['best_station']),
            (tr('Top Customer by Spend'), highlights['top_customer']), (tr('Critical Supplies'), highlights['critical_supplies']),
            (tr('Exceptions / Delays'), highlights['exceptions']),
        ]
        ...
        summary.write(21, 1, tr('Revenue Trend'), section_fmt)
        summary.write(21, 2, tr('Revenue'), section_fmt)
        ...
        summary.write(21, 4, tr('Payment Method'), section_fmt)
        summary.write(21, 5, tr('Amount'), section_fmt)
        ...
        if data.get('revenue_trend'):
            ...
            chart.add_series({
                'name': tr('Revenue'),
                'categories': f"='{summary_ref}'!$B$23:$B${22 + len(data['revenue_trend'])}",
                'values': f"='{summary_ref}'!$C$23:$C${22 + len(data['revenue_trend'])}",
                'fill': {'color': blue}, 'border': {'none': True},
            })
            chart.set_title({'name': tr('Revenue Trend')})
        ...
        if data.get('payment_methods'):
            ...
            pie.add_series({
                'name': tr('Payment Methods'),
                'categories': f"='{summary_ref}'!$E$23:$E${22 + len(data['payment_methods'])}",
                'values': f"='{summary_ref}'!$F$23:$F${22 + len(data['payment_methods'])}",
                'data_labels': {'percentage': True},
            })
            pie.set_title({'name': tr('Payment Methods')})
            pie.set_hole_size(55)
            summary.insert_chart('H31', pie, {'x_scale': 1.1, 'y_scale': 1.0})
        summary.freeze_panes(5, 1)
        summary.write('B40', tr('Official revenue source: paid Point of Sale orders.'), note_fmt)

        # Operations
        def op_conditional(sheet, first, last):
            if last >= first:
                sheet.conditional_format(first, 9, last, 9, {'type': 'text', 'criteria': 'containing', 'value': tr('Finished'), 'format': success_fmt})
                sheet.conditional_format(first, 9, last, 9, {'type': 'text', 'criteria': 'containing', 'value': tr('Cancelled'), 'format': danger_fmt})
                sheet.conditional_format(first, 9, last, 9, {'type': 'text', 'criteria': 'containing', 'value': tr('In Progress'), 'format': warning_fmt})
        operations_sheet, _, _ = add_table_sheet(
            tr('Operations'), tr('Wash Operations'),
            [tr('Date'), tr('Time'), tr('Ticket'), tr('Plate'), tr('Vehicle'), tr('Customer'), tr('Service'), tr('Station'), tr('Duration (min)'), tr('Status')],
        ...
        operations_sheet.write(len(data.get('operations') or []) + 7, 7, tr('Total Operations'), total_fmt)
        operations_sheet.write(len(data.get('operations') or []) + 7, 8, len(data.get('operations') or []), total_fmt)

        # Revenue & Payments
        revenue_sheet = workbook.add_worksheet(revenue_name)
        write_report_header(revenue_sheet, tr('Revenue & Payments'), 8)
        ...
        tx_headers = [tr('Date'), tr('Time'), tr('POS Order'), tr('Customer'), tr('Service'), tr('Qty'), tr('Unit Price'), tr('Amount'), tr('Payment Method')]
        ...
        summary_col = 10
        revenue_sheet.write(5, summary_col, tr('Payment Method'), header_fmt); revenue_sheet.write(5, summary_col + 1, tr('Amount'), header_fmt)
        ...
        revenue_sheet.write(5, summary_col + 3, tr('Period'), header_fmt); revenue_sheet.write(5, summary_col + 4, tr('Revenue'), header_fmt)
        ...
        if data.get('payment_methods'):
            pie = workbook.add_chart({'type': 'doughnut'})
            pie.add_series({'categories': f"='{revenue_ref}'!$K$7:$K${6 + len(data['payment_methods'])}", 'values': f"='{revenue_ref}'!$L$7:$L${6 + len(data['payment_methods'])}", 'data_labels': {'percentage': True}})
            pie.set_title({'name': tr('Payment Mix')}); pie.set_hole_size(55)
            revenue_sheet.insert_chart('K15', pie, {'x_scale': 1.0, 'y_scale': 0.9})
        if data.get('revenue_trend'):
            chart = workbook.add_chart({'type': 'line'})
            chart.add_series({'name': tr('Revenue'), 'categories': f"='{revenue_ref}'!$N$7:$N${6 + len(data['revenue_trend'])}", 'values': f"='{revenue_ref}'!$O$7:$O${6 + len(data['revenue_trend'])}", 'line': {'color': blue, 'width': 2.25}, 'marker': {'type': 'circle', 'size': 5, 'border': {'color': blue}, 'fill': {'color': '#FFFFFF'}}})
            chart.set_title({'name': tr('Revenue Trend')}); chart.set_legend({'none': True})
            revenue_sheet.insert_chart('N15', chart, {'x_scale': 1.15, 'y_scale': 0.9})
        revenue_sheet.write(tx_end + 2, 6, tr('Total Revenue'), total_fmt); revenue_sheet.write(tx_end + 2, 7, data.get('kpis', {}).get('total_revenue', 0), total_money_fmt)
        ...
        add_table_sheet(tr('Services'), tr('Services Performance'), [tr('Service'), tr('Times Sold'), tr('Revenue'), tr('Average Duration'), tr('Sales %')], data.get('services') or [], ['name', 'value', 'revenue', 'average_duration', 'sales_percent'], money_keys=('revenue',), percent_keys=('sales_percent',), widths=[30, 14, 18, 20, 14], conditional=service_conditional)

        def station_conditional(sheet, first, last):
            if last >= first:
                sheet.conditional_format(first, 3, last, 3, {'type': 'data_bar', 'bar_color': blue})
        add_table_sheet(tr('Stations'), tr('Station Performance'), [tr('Station'), tr('Type'), tr('Cars Served'), tr('Utilization %'), tr('Busy Minutes'), tr('Average Duration')], data.get('stations') or [], ['name', 'station_type', 'cars_served', 'utilization', 'busy_minutes', 'average_duration'], percent_keys=('utilization',), widths=[28, 16, 14, 16, 16, 18], conditional=station_conditional)

        def customer_conditional(sheet, first, last):
            if last >= first:
                sheet.conditional_format(first, 2, last, 2, {'type': 'data_bar', 'bar_color': green})
        add_table_sheet(tr('Customers'), tr('Customer Performance'), [tr('Customer'), tr('Visits'), tr('Total Spend'), tr('Average Spend'), tr('Last Visit'), tr('Most Used Service')], data.get('customers') or [], ['name', 'visits', 'total_spend', 'average_spend', 'last_visit', 'top_service'], money_keys=('total_spend', 'average_spend'), widths=[28, 12, 18, 18, 20, 30], conditional=customer_conditional)
        ...
        add_table_sheet(tr('Supplies'), tr('Cleaning Supplies'), [tr('Material'), tr('On Hand'), tr('Minimum'), tr('UoM'), tr('Remaining %')], data.get('supplies') or [], ['name', 'available_qty', 'minimum_qty', 'uom', 'remaining_percent'], percent_keys=('remaining_percent',), widths=[38, 14, 14, 12, 16], conditional=supply_conditional)
        ...
        add_table_sheet(tr('Exceptions'), tr('Exceptions & Delays'), [tr('Type'), tr('Reference'), tr('Vehicle'), tr('Issue')], data.get('exceptions') or [], ['type', 'reference', 'detail', 'message'], widths=[16, 22, 18, 48], conditional=exception_conditional)
    ...
    def export_pdf(self, **params):
        ...
        def on_page(canvas, doc):
            ...
            canvas.drawRightString(width - 12 * mm, 5.2 * mm, f"{tr('Page')} {canvas.getPageNumber()} | {tr('Generated')} {meta['generated_at']}")
            canvas.restoreState()

        doc = SimpleDocTemplate(
            output, pagesize=landscape(A4), rightMargin=12 * mm, leftMargin=12 * mm,
            topMargin=12 * mm, bottomMargin=14 * mm,
            title=tr('Crystal Clean - Management Report'), author=meta['generated_by'],
        )
        styles = getSampleStyleSheet()
        title_style = ParagraphStyle('CwTitle', parent=styles['Title'], fontName=font_name, fontSize=23, leading=27, textColor=colors.HexColor('#0D2744'), alignment=TA_RIGHT if rtl else TA_LEFT, spaceAfter=4)
        subtitle_style = ParagraphStyle('CwSubtitle', parent=styles['Normal'], fontName=font_name, fontSize=9, leading=12, textColor=colors.HexColor('#607A94'), alignment=TA_RIGHT if rtl else TA_LEFT, spaceAfter=6)
        section_style = ParagraphStyle('CwSection', parent=styles['Heading2'], fontName=font_name, fontSize=15, leading=18, textColor=colors.HexColor('#0D2744'), alignment=TA_RIGHT if rtl else TA_LEFT, spaceBefore=4, spaceAfter=7)
        ...
        if logo:
            ...
            brand.append(Paragraph('Crystal Clean', title_style))
        brand.append(Paragraph(f'<b>{html.escape(tr("Crystal Clean - Management Report"))}</b><br/><font size="9">{html.escape(tr("Complete car wash management overview"))}</font>', title_style))
        ...
        story += [brand_table, Spacer(1, 4 * mm), Paragraph(tr('Executive Summary'), section_style), Paragraph(f"<b>{html.escape(tr('Period'))}:</b> {html.escape(data.get('period_label') or '-')}<br/><b>{html.escape(tr('Generated By'))}:</b> {html.escape(meta['generated_by'])} &nbsp;&nbsp; <b>{html.escape(tr('Generated At'))}:</b> {meta['generated_at']}<br/><b>{html.escape(tr('Applied Filters'))}:</b> {html.escape(meta['filters'])}", subtitle_style)]

        kpis = data.get('kpis') or {}
        kpi_rows = [
            [tr('Total Cars'), tr('Finished Cars'), tr('Waiting Cars'), tr('Total Revenue'), tr('Average Ticket'), tr('Unique Customers')],
        ...
        highlight_data = [
            [tr('Top Service'), highlights['top_service'], tr('Highest Utilization Station'), highlights['best_station']],
            [tr('Top Customer'), highlights['top_customer'], tr('Critical Supplies'), highlights['critical_supplies']],
            [tr('Exceptions / Delays'), highlights['exceptions'], tr('Official Revenue Source'), tr('Paid POS Orders')],
        ]
        ...
        story += [highlight_table, PageBreak(), Paragraph(tr('Revenue & Payments'), section_style)]
        ...
        if trend:
            ...
            drawing.add(String(36, 162, tr('Revenue Trend'), fontName=font_name, fontSize=10, fillColor=colors.HexColor('#0D2744')))
            drawing.add(chart)
        ...
        if payments:
            ...
            drawing.add(String(42, 162, tr('Payment Methods'), fontName=font_name, fontSize=10, fillColor=colors.HexColor('#0D2744')))
            drawing.add(pie)
            charts.append(drawing)
        if charts:
            story.append(Table([charts], colWidths=[125 * mm] * len(charts)))
            story.append(Spacer(1, 4 * mm))
        payment_rows = [[tr('Payment Method'), tr('Amount')]] + [[row.get('name'), f"{float(row.get('amount') or 0):,.2f} {currency}"] for row in payments]
        ...
        story += [payment_table, PageBreak(), Paragraph(tr('Wash Operations'), section_style)]

        op_headers = [tr('Date'), tr('Time'), tr('Ticket'), tr('Plate'), tr('Vehicle'), tr('Customer'), tr('Service'), tr('Station'), tr('Duration'), tr('Status')]
        op_rows = [op_headers]
        for row in data.get('operations') or []:
            op_rows.append([row.get('date'), row.get('time'), row.get('ticket'), row.get('plate'), row.get('vehicle'), row.get('customer'), row.get('service'), row.get('station'), f"{float(row.get('duration') or 0):.0f} {tr('min')}", row.get('status_label')])
        ...
        story += [op_table, PageBreak(), Paragraph(tr('Service Performance'), section_style)]

        service_rows = [[tr('Service'), tr('Times Sold'), tr('Revenue'), tr('Average Duration'), tr('% of Total Sales')]]
        for row in data.get('services') or []:
            service_rows.append([row.get('name'), row.get('value'), f"{float(row.get('revenue') or 0):,.2f} {currency}", f"{float(row.get('average_duration') or 0):.1f} {tr('min')}", f"{float(row.get('sales_percent') or 0):.1f}%"])
        ...
        story += [service_table, PageBreak(), Paragraph(tr('Station Performance'), section_style)]

        station_rows = [[tr('Station'), tr('Type'), tr('Cars Served'), tr('Utilization'), tr('Busy Minutes'), tr('Average Duration')]]
        for row in data.get('stations') or []:
            station_rows.append([row.get('name'), self._localized_technical_label(row.get('station_type')), row.get('cars_served'), f"{float(row.get('utilization') or 0):.1f}%", f"{float(row.get('busy_minutes') or 0):.1f}", f"{float(row.get('average_duration') or 0):.1f} {tr('min')}"])
        ...
        story += [station_table, PageBreak(), Paragraph(tr('Top Customers'), section_style)]

        customer_rows = [[tr('Customer'), tr('Visits'), tr('Total Spend'), tr('Average Spend'), tr('Last Visit'), tr('Most Used Service')]]
        ...
        story += [customer_table, PageBreak(), Paragraph(tr('Supplies & Exceptions'), section_style)]

        supply_rows = [[tr('Material'), tr('On Hand'), tr('Minimum'), tr('UoM'), tr('Remaining')]]
        ...
        exception_rows = [[tr('Type'), tr('Reference'), tr('Vehicle'), tr('Issue')]]
        for row in data.get('exceptions') or []:
            exception_rows.append([self._localized_technical_label(row.get('type')), row.get('reference'), row.get('detail'), row.get('message')])
    ...
    def print_report(self, **params):
        ...
        meta = self._generated_meta(data)
        kpis = data.get('kpis') or {}
        operations = [[row.get('date'), row.get('time'), row.get('ticket'), row.get('plate'), row.get('vehicle'), row.get('customer'), row.get('service'), row.get('station'), f"{float(row.get('duration') or 0):.0f} {tr('min')}", row.get('status_label')] for row in data.get('operations') or []]
        services = [[row.get('name'), row.get('value'), f"{float(row.get('revenue') or 0):,.2f} {data.get('currency_code')}", f"{float(row.get('average_duration') or 0):.1f} {tr('min')}", f"{float(row.get('sales_percent') or 0):.1f}%"] for row in data.get('services') or []]
        stations = [[row.get('name'), self._localized_technical_label(row.get('station_type')), row.get('cars_served'), f"{float(row.get('utilization') or 0):.1f}%", f"{float(row.get('average_duration') or 0):.1f} {tr('min')}"] for row in data.get('stations') or []]
        ...
        exceptions = [[self._localized_technical_label(row.get('type')), row.get('reference'), row.get('detail'), row.get('message')] for row in data.get('exceptions') or []]
        direction = 'rtl' if rtl else 'ltr'
        language = 'ar' if rtl else 'en'
        text_align = 'right' if rtl else 'left'
        page = f'''<!doctype html>
<html lang="{language}" dir="{direction}"><head><meta charset="utf-8"/><title>{html.escape(tr('Crystal Clean - Management Report'))}</title>
<style>
@page {{ size: A4 landscape; margin: 12mm; }}
body {{ font-family: Arial, sans-serif; color:#173b5d; margin:0; background:#fff; direction:{direction}; text-align:{text_align}; }}
...
h1 {{ margin:0; color:#0d2744; font-size:26px; }} .meta {{ color:#607a94; font-size:11px; line-height:1.55; text-align:{text_align}; }}
...
h2 {{ color:#0d2744; font-size:16px; margin:18px 0 8px; }} table {{ width:100%; border-collapse:collapse; font-size:9px; margin-bottom:15px; }} th {{ background:#0d2744; color:white; padding:6px; text-align:{text_align}; }} td {{ border:1px solid #dbe6ef; padding:5px; text-align:{text_align}; }} tr:nth-child(even) td {{ background:#f8fbfe; }}
.actions {{ display:none; }} @media print {{ .actions {{ display:none!important; }} }}
</style></head><body>
<header><div><h1>{html.escape(tr('Crystal Clean - Management Report'))}</h1><div>{html.escape(data.get('period_label') or '')}</div></div><div class="meta">{html.escape(tr('Generated By'))}: {html.escape(meta['generated_by'])}<br/>{html.escape(tr('Generated At'))}: {meta['generated_at']}<br/>{html.escape(meta['filters'])}</div></header>
<section class="kpis">
<div class="kpi">{html.escape(tr('Total Cars'))}<b>{kpis.get('total_cars',0)}</b></div><div class="kpi">{html.escape(tr('Finished Cars'))}<b>{kpis.get('finished_cars',0)}</b></div><div class="kpi">{html.escape(tr('Waiting Cars'))}<b>{kpis.get('waiting_cars',0)}</b></div><div class="kpi">{html.escape(tr('Revenue'))}<b>{kpis.get('total_revenue',0):,.2f} {data.get('currency_code')}</b></div><div class="kpi">{html.escape(tr('Average Ticket'))}<b>{kpis.get('average_ticket',0):,.2f}</b></div><div class="kpi">{html.escape(tr('Customers'))}<b>{kpis.get('unique_customers',0)}</b></div>
</section>
<h2>{html.escape(tr('Revenue & Payments'))}</h2>{self._html_table([tr('Payment Method'),tr('Amount')], payments)}
<h2>{html.escape(tr('Wash Operations'))}</h2>{self._html_table([tr('Date'),tr('Time'),tr('Ticket'),tr('Plate'),tr('Vehicle'),tr('Customer'),tr('Service'),tr('Station'),tr('Duration'),tr('Status')], operations)}
<h2>{html.escape(tr('Service Performance'))}</h2>{self._html_table([tr('Service'),tr('Times Sold'),tr('Revenue'),tr('Average Duration'),tr('Sales %')], services)}
<h2>{html.escape(tr('Station Performance'))}</h2>{self._html_table([tr('Station'),tr('Type'),tr('Cars Served'),tr('Utilization'),tr('Average Duration')], stations)}
<h2>{html.escape(tr('Top Customers'))}</h2>{self._html_table([tr('Customer'),tr('Visits'),tr('Total Spend'),tr('Last Visit'),tr('Most Used Service')], customers)}
<h2>{html.escape(tr('Cleaning Supplies'))}</h2>{self._html_table([tr('Material'),tr('On Hand'),tr('Minimum'),tr('UoM'),tr('Remaining')], supplies)}
<h2>{html.escape(tr('Exceptions & Delays'))}</h2>{self._html_table([tr('Type'),tr('Reference'),tr('Vehicle'),tr('Issue')], exceptions)}
