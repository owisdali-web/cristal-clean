...
class TestCarWashDashboard(TransactionCase):
    def test_frontend_translation_bundle_includes_dashboard_module(self):
        modules = self.env['ir.http']._get_translation_frontend_modules_name()
        self.assertIn('car_wash_dashboard', modules)
    ...
    def test_payload_has_only_real_enabled_stations(self):
        Workcenter = self.env['mrp.workcenter']
        ...
        self.assertEqual(payload['dashboard_version'], '6.5-business-analytics')
        self.assertGreaterEqual(len(payload['stations']), 2)
        self.assertFalse(any(station.get('is_placeholder') for station in payload['stations']))
        ...
        self.assertEqual(payload['station_configuration']['target_station_count'], payload['station_configuration']['real_station_count'])
